...
As a simple quantitative proxy for label noise, I examine prediction uncertainty on the baseline test set. Around 6.8\% of studies fall into an ambiguous band (Effusion probability 0.4--0.6), with similar rates for F (7.1\%) and M (6.6\%). Overall error rate at the 0.5 threshold is 21.0\% (F: 20.6\%, M: 21.3\%), and high-confidence errors (probability $\geq 0.8$ but wrong) are also similar across sex (F: 9.2\%, M: 9.1\%). These proxies suggest comparable label/prediction uncertainty across groups, though the absolute error rate highlights expected label noise in a weakly labeled dataset.
...
\end{itemize}
Gaps are reported as absolute differences in probabilities (e.g., $\eoGap = 0.014$ means a 1.4 percentage point TPR gap).
...
\begin{table}[h]
  \centering
  \caption{Baseline metrics on the test set for Effusion.}
  \label{tab:baseline_fairness}
  \begin{tabular}{lcccc}
    \toprule
    Group & AUROC & AUPRC & TPR@95\%Spec & Brier \\
    \midrule
    Overall & 0.883 & 0.524 & 0.478 & -- \\
    Sex = F & 0.884 & 0.525 & 0.464 & 0.147 \\
    Sex = M & 0.882 & 0.525 & 0.486 & 0.149 \\
    \bottomrule
  \end{tabular}
\end{table}

Using \texttt{fairness\_sex/fairness\_summary.json}, the baseline fairness gaps are:
\[
  \dpGap = 0.0002,\quad
  \eoGap = 0.0140,\quad
  \eodsGap = 0.0140.
...
\begin{table}[h]
  \centering
  \caption{Macro test metrics and fairness gaps across methods.}
...
Using \texttt{main.py calibrate}, the baseline expected calibration error (ECE) decreases from 0.0884 to 0.0473 after temperature scaling with $T\approx 1.49$, while AUROC and AUPRC remain unchanged. Reliability diagrams (not shown) confirm that overconfidence at high predicted probabilities is reduced.

Thresholds for 95\% specificity differ only slightly between F and M, and between methods (e.g., baseline thresholds $\approx 0.90$ for both groups). This suggests that a single global threshold is not strongly unfair for sex in this dataset, although group-specific operating points could still be used in a deployment setting.
...
\subsection{Task D \& E -- Explainability and Interplay with Fairness}
To make the classifier's decisions more interpretable, I compute gradient-based class activation maps (CAMs) for the Effusion logit. Intuitively, these maps highlight image regions where small increases in pixel intensity would most increase the model's confidence in the positive class. Warm colors (red/yellow) indicate regions that strongly support the effusion prediction, while cool colors (blue) indicate less influential areas.

For each of the three models (baseline, reweight, GroupDRO), I selected a fixed set of 16 test radiographs and produced CAM overlays. Figure~\ref{fig:case1} shows an example high-confidence positive case. All three models focus their attention on the lower right hemithorax and costophrenic angle, which is consistent with a right-sided effusion and therefore clinically plausible. The mitigated models (especially GroupDRO) produce slightly more compact and lung-confined hotspots, whereas the baseline CAM also spills into mediastinal structures.

In a second case study (Figure~\ref{fig:case2}), the baseline model assigns moderate effusion probability to a study without clear radiographic effusion. Its CAM partially highlights the cardiac silhouette and upper mediastinum rather than the pleural recesses, suggesting a spurious association with central brightness. Reweighting reduces the overall confidence and shifts attention slightly towards the lung bases, while GroupDRO further down-weights this case and produces a more diffuse, lower-intensity CAM. This example illustrates that fairness-oriented training can also change where the model ``looks'', potentially reducing reliance on shortcut features.

Under mild noise and brightness/contrast perturbations, CAMs remain qualitatively similar in many high-confidence positive cases, but become more unstable for borderline examples: hotspots sometimes shift from lung fields to ribs or soft tissue without large changes in predicted probability. Combined with the strong drop in TPR@95\% specificity under perturbations, this underlines the need to interpret saliency maps with caution and to pair them with robustness checks.
...
\begin{figure}[h]
  ...
  \caption{Class activation maps for a high-confidence Effusion case (left to right: baseline, reweight, GroupDRO). All models highlight the right hemithorax; mitigations produce more compact, lung-focused saliency.}
  \label{fig:case1}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Borderline case where the baseline CAM focuses on mediastinal structures; mitigated models lower the predicted probability and shift attention towards the lung bases.}
  \label{fig:case2}
\end{figure}
